...
const util = {
  ...
  reFolder(dir, filterFile) {
    let res = [];
    let item = [];
    let fileArr = fs.readdirSync(path.resolve(__dirname, dir));
    fileArr.map(x => {
      if (x.indexOf('.') < 0) {
  ...
  getList(tokenConfig) {
    return new Promise((resolve, reject) => {
      let bucketManager = new qiniu.rs.BucketManager(mac, tokenConfig.qiniuConfig);
      let options = {
        prefix: prefix
      };
  ...
  uploadFile(file, tokenConfig) {
    let { uploadToken, qiniuConfig, formUploader, putExtra } = tokenConfig;
    return new Promise((resolve, reject) => {
      formUploader.putFile(uploadToken, file.key, file.localFile, putExtra, function(error, body, info) {
        !error || (info && info.statusCode == 200) ? resolve({ body, info }) : resolve({ error, body, info });
      });
    });
  },
  // 删除单个文件
  deleteFile(file, { qiniuConfig }) {
    return new Promise((resolve, reject) => {
      ...
      bucketManager.batch([qiniu.rs.deleteOp(argList.bucket, file.key)], function(error, body, info) {
        !error || (info && info.statusCode == 200) ? resolve({ body, info }) : resolve({ error, body, info });
      });
    });
  },
  ...
  showLog(action, res, name, num1, num2) {
    let showState = res.error ? 'error' : 'success';
...
class QiniuClass {
  ...
  async _screen() {
    ...
    // 判断需要删除和上传的文件
    let [needUpload, needDelete] = util.deleteDiff(this.files, this.onlineList);
  ...
  async _delete() {
    ...
    if (this.needDelete.length > 0) {
      ...
      for (let x in this.needDelete) {
        let tokenConfig = util.createToken();
        let res = await util.deleteFile(this.needDelete[x], tokenConfig);
        this.deletes.push(res);
        util.showLog('delete', res, this.needDelete[x].key, this.deletes.length, this.needDelete.length);
      }
    }
  }
  // 上传本地图片
  async _upload() {
    ...
    if (this.needUpload.length > 0) {
      ...
      for (let x in this.needUpload) {
        let tokenConfig = util.createToken();
        let res = await util.uploadFile(this.needUpload[x], tokenConfig);
        this.uploads.push(res);
        util.showLog('upload', res, this.needUpload[x].key, this.uploads.length, this.needUpload.length);
      }
    }
  }
  // 刷新cdn文件
  async _refresh() {
    ...
    if (this.needUpload.length > 0 && cdn) {
      ...
      for (let x in this.needUpload) {
        let res = await util.refreshFile(this.needUpload[x]);
        this.refreshs.push(res);
        util.showLog('refresh', res, this.needUpload[x].key, this.refreshs.length, this.needUpload.length);
      }
    }
  }
  // 统计文件
  async _statistical() {
    let [deleteSuccess, deleteError] = this.deletes.reduce(
      (a, b) => {
        b.error ? a[1]++ : a[0]++;
        return a;
      },
    ...
    let [uploadSuccess, uploadError] = this.uploads.reduce(
      (a, b) => {
        b.error ? a[1]++ : a[0]++;
        return a;
      },
    ...
    let [refreshSuccess, refreshError] = this.refreshs.reduce(
      (a, b) => {
        b.error ? a[1]++ : a[0]++;
        return a;
      },
    ...
    console.log(`\n删除文件 -->  total:${this.needDelete.length}  success:${deleteSuccess}  error:${deleteError}`);
    console.log(`上传文件 -->  total:${this.needUpload.length}  success:${uploadSuccess}  error:${uploadError}`);
    console.log(`刷新文件 -->  total:${this.needUpload.length}  success:${refreshSuccess}  error:${refreshError}\n\n\n\n\n\n`);
  }
  async start() {
    await this._screen();
